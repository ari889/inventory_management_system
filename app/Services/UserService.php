...
class UserService extends BaseService
{
    ...
    {
        if ($user->avatar) {
            return "<img src='storage/" . USER_AVATAR_PATH . $user->avatar . "' alt='' style='width: 50px'>";
        } else {
            return "<img src='images/" . ($user->gender == 1 ? 'male' : 'female') . ".svg' alt='' style='width: 50px'>";
        }
    }

    public function store_or_update_data(Request $request)
    {
        $collection = collect($request->validated())->except('password_confirmation');
