        ...
        if(permission('product-access')){
            if($request->ajax()){
                ...
                foreach ($list as $value) {
                    ...
                    $row[] = permission('product-edit') ? change_status($value->id, $value->status, $value->title) : STATUS_LABEL[$value->status];
        ...
        if($request->ajax()){
            ...
            if($product_data){
                ...
                if($request->type == 'sale'){
                    $warehouse_product = WarehouseProduct::where(['warehouse_id' => $request->warehouse_id, 'product_id' => $product_data->id])->first();
