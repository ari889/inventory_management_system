...
class AccountFormRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $rules['name'] = ['required', 'string', 'unique:accounts,account_no'];
        $rules['initial_balance'] = ['nullable', 'numeric', 'gt:0'];
        $rules['note'] = ['nullable', 'string'];
        if(request()->update_id){
            $rules['account_no'][2] = 'unique:accounts,account_no,'.request()->update_id;
            $rules['name'][2] = 'unique:accounts,account_no,'.request()->update_id;
        }
        return $rules;
    }
