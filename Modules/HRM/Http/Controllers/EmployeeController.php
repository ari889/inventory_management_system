...
class EmployeeController extends BaseController
{
    ...
    {
        if ($employee->image) {
            return "<img src='storage/" . EMPLOYEE_IMAGE_PATH . $employee->image . "' alt='' style='width: 50px'>";
        } else {
            return "<img src='images/male.svg' alt='' style='width: 50px'>";
        }
    }
    ...
    public function delete(Request $request){
        if($request->ajax()){
            if(permission('employee-delete')){
                ...
                $output = $this->delete_message($employee);
            }else{
                $output = $this->access_blocked();
            }
            return response()->json($output);
